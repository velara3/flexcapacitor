package com.flexcapacitor.states {
	...
	public class AddItems extends OverrideBase 
	{
		...
		protected function addItemsToContentHolder(dest:IVisualElementContainer, items:Array):Boolean
		{
			...
			// startIndex = -1 means we want to add it at the end 
			// so we are going to use addElement because that does not throw range error
			// when element is already child of destination
			if (startIndex == -1) {
				
				// we could also try removing them all first and then adding them to the end??
			...
			for (var g:int = 0; g < items.length; g++) {
				dest.addElementAt(items[g], startIndex + g);
			}
