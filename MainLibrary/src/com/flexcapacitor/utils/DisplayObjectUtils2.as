...
package com.flexcapacitor.utils {
	 ...
	 * 
	 * 2014 - Found that LayoutDebugHelper based on an excluded Flex class has methods to get the
	 * correct or expected bounds and position of UIComponents. 
	 * */
	public class DisplayObjectUtils2 {
		...
		public function DisplayObjectUtils2() {
	   ...
	   /**
		 ...
		 * can listen to mouse events for drag over and drag and drop events over groups. 
		...
		public static function addGroupMouseSupport(group:GroupBase, groupsDictionary:Dictionary = null):void {
			...
			var groupOptions:GroupOptions = new GroupOptions(group.mouseEnabledWhereTransparent);
			var mouseEnabledWhereTransparent:Boolean = groupOptions.mouseEnabledWhereTransparent;
			if(!group.hasEventListener(MouseEvent.MOUSE_OVER)) {
				group.addEventListener(MouseEvent.MOUSE_OVER, transparentGroupMouseHandler, false, 0, true);
			}
			groupsDictionary[group] = groupOptions;
			group.mouseEnabledWhereTransparent = true;
			
			trace("Adding mouse support to group: " + ClassUtils.getClassName(group));
		}
		...
		public static function removeGroupMouseSupport(group:GroupBase, groupsDictionary:Dictionary = null):void {
			if (!groupsDictionary) groupsDictionary = applicationGroups;
			var mouseEnabledWhereTransparent:Boolean;
			
			//TypeError: Error #1010: A term is undefined and has no properties. (applicationGroups)
			if (group in groupsDictionary) {
				mouseEnabledWhereTransparent = groupsDictionary[group].mouseEnabledWhereTransparent;
				group.mouseEnabledWhereTransparent = mouseEnabledWhereTransparent;
				if (!mouseEnabledWhereTransparent) {
					group.removeEventListener(MouseEvent.MOUSE_OVER, transparentGroupMouseHandler);
				}
				groupsDictionary[group] = null;
				delete groupsDictionary[group];
				
				trace("Removing mouse support to group: " + ClassUtils.getClassName(group));
			}
		}
		
		/**
		 * Handler for mouse events on groups. Transparent Group containers need at least one mouse event 
		 * listener to track mouse events over it. This is the handler we add to the group. It doesn't do anything. 
		 * */
		public static function transparentGroupMouseHandler(event:MouseEvent):void {
			// this is used to enable mouse events where transparent 
			trace("Class utils. Transparent group mouse out handler: " + ClassUtils.getClassName(event.currentTarget));
		}
